...
void f_export_histograms(struct o_string *output, struct s_calibration_charts *charts) {
	v_chart_split_x = 2;
	v_chart_split_y = 3;
	p_export_histograms_singleton(output, d_false, d_false, d_trb_event_vas, e_pdf_page_middle, "HIST", "TTTTPP", charts->pedestal, charts->sigma_raw,
...
int main (int argc, char *argv[]) {
	...
	d_try {
		...
		if ((calibration) && (output)) {
			...
			charts.pedestal->GetYaxis()->SetRangeUser(0, 500);
			...
			charts.sigma_raw->GetYaxis()->SetRangeUser(0, 10);
			...
			charts.sigma->GetYaxis()->SetRangeUser(0, 5);
			...
			charts.gain->GetYaxis()->SetRangeUser(0, 2);
