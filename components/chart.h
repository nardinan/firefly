...
typedef struct s_chart {
	GtkWidget *plane;
	int head, elements, last_width, last_height, histogram, show_borders, border_x, border_y;
	struct {
		float x_axis, y_axis;
	} normalized;
	...
	struct s_chart_value values[d_chart_bucket];
} s_chart;
...
extern void p_chart_normalize_switch(struct s_chart *chart, unsigned int left, unsigned int right);
extern void p_chart_normalize_sort(struct s_chart *chart);
